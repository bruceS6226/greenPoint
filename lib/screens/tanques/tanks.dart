        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () async {
              ...
              final prefs = await SharedPreferences.getInstance();
              final machine = jsonDecode(prefs.getString('SpecificMachine')!);
              ...
              final payload = {
                ...
                "pistons": pistons.map((e) => e.toJson()).toList(),
              };

              try {
                final bytes = await fileService.generateTankExcel(payload);
                final dir = await getApplicationDocumentsDirectory();
                final path = '${dir.path}/${machine['name']}.xlsx';
        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () async {
              ...
              try {
                final bytes = await fileService.generateTankPdf(payload);
                final dir = await getApplicationDocumentsDirectory();
                final path = '${dir.path}/${machine['name']}.pdf';
                final file = File(path);
                await file.writeAsBytes(bytes);
                ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  Positioned(
                    ...
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        double levelWidth = (tank.level / 200) * maxWidth;
